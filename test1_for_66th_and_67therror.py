...
def validate_amount(main_root):
    ...
    if error != "":
        ...
        #             match = re.search(r"\$\s?\d[\d,]*\.?\d{0,2}", combined_line)
        ...
        #             # match = re.search(r"\$\d[\d,]*\.\d{2}", line)
        #             match = re.search(r"\$\s?\d[\d,]*\.?\d{0,2}", line)
        #             if match:
            ...
            try:
                ...
                for item in results:
                    ...
                    if invoice_no and invoice_no not in invoice_folder_invoices:
                        output += f"📂 Invoice {invoice_no} from ENTRY SUMMARY pdf paru has no matching file in Invoice Folder.\n"
