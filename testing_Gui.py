...
def validate_amount():
    ...
    if error != "":
        ...
        def extract_invoice_fields(text_lines, source_file):
            global total_mpf, total_hmf, total_duty, total_entered_value
            extracted = []
            current_item = {}
                ...
                for invoice, fob in invoice_fob_list:
                    ...
                    for item in results:
                        if str(item["Invoice Number"]).strip() == str(invoice).strip():
                            ...
                            extracted_value = clean(item.get("Extracted Value", "0"))  # NEW LINE

                            if (abs(fob - entered_value) <= 0.5 and
                                abs(fob - invoice_value) <= 0.5 and
                                abs(fob - extracted_value) <= 0.5):  # NEW CONDITION
